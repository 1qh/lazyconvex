...
`setup()` returns `pq`, `q`, and `m` — thin wrappers around Convex’s query/mutation
builders that inject auth context and helpers.
Use them when generated CRUD isn’t enough.
...
You can also drop to raw Convex `action`/`mutation`/`query` when you don’t need
lazyconvex’s auth context:
...
The generated `where` clauses (`$gt`, `$lt`, `$between`, `or`) use runtime `.filter()`
after fetching documents.
This works well for tables under ~1,000 documents.
When a table grows past that, you’ll see the `RUNTIME_FILTER_WARN_THRESHOLD` warning in
logs.
...
Keep generated CRUD for mutations and simple reads, add custom indexed queries only for
hot paths.
...
The `c` context object is fully typed.
Here’s what each builder gives you:
...
Keep `crud()` for standard operations, add custom endpoints for specialized reads:
...
All endpoints — generated and custom — are exported from the same file.
The frontend imports them all from `api.blog`:
...
export const {
  ...
  list = pq({
    // replace generated list with indexed version
